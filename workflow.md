...
Alice                                     Bob
  ...
  │   (keygen + empty vault)                │   (keygen + empty vault)
  │                                         │
  │── 2. Add credentials                    │
  │   (AES-GCM encrypt + ElGamal sign)      │
  │                                         │
  │── 3. Retrieve credential                │
  │   (verify sig → decrypt → display)      │
  │                                         │
  │── 4. Export ────────────────────────────│── 4. Import
  │   DH keygen + sign DH pub key           │   Verify sig
  │   ◄──────────────── DH pub key + sig ───│   DH keygen + sign
  │   Verify sig                            │
  │   Compute shared secret ────────────────│   Compute shared secret
  │   Derive session key                    │   Derive session key
  │   Encrypt vault with session key        │
  │   Sign encrypted data                   │
  │   ─────────────── transfer package ────►│   Verify sig
  │                                         │   Decrypt with session key
  │                                         │   Re-encrypt with Bob's master pw
  │                                         │   Sign + save vault
```
